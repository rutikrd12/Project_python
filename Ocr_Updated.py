...
def extract_tesseract(filename):   # use to extract text from the file
        temp_dir = mkdtemp()
...
def convert_recursive(source, destination, count):    # use for pdf file counter
    pdfCounter = 0
    ...
    for dirpath, dirnames, files in os.walk(source):
        for name in files:
            ...
            output_directory = os.path.join(destination, relative_directory)   #joined the output derectory-----
...
def convert(sourcefile, destination_file, count, pdfCounter):   #extracting the text from file & stored it into text file
...
count = 0
#--------------------------------------------------------------------------------------------------------
print('    --------------PDF to TXT file Converter--------------    ')
print('                                                             ')
#---------------------------------------------------------------------------------------------------------
dir_path = os.path.dirname(os.path.realpath(__file__))  # use when source or distination is empty
print('Source file or folder of PDF(s) [' + dir_path + ']:')
print('(Press [Enter] for current working directory)')

source = input("Enter the pdf files location:")  
if source == '':
    source = dir_path
...
destination = input("Enter the output text files location:")
if destination == '':
